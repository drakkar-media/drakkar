<script lang="ts">
  ...
  const tasks: TaskDef[] = [
    ...
    { id: 'nzb_health_check', label: 'Deep NZB Article Check', description: 'Probe first/last NNTP articles for available items and reset missing-article or sample-only publications for re-queue.', group: 'Maintenance', interval: '168h', manual: true, run: async () => { const r = await api.maintenance('nzb-health-check'); return `scanned ${r.scannedRows}, reset ${r.resetItems}`; } },
