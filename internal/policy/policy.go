...
func Classify(reason string) FailureKey {
	...
	switch {
	// NNTP 430 (and 423) mean "no such article" per RFC 3977 and Newshosting's
	// own support docs — the article is confirmed gone (past retention or
	// removed), not a transient throttle. An earlier version of this
	// classified 430 as a transient throttle; see isThrottleLikeErr in
	// internal/nntp/circuit_breaker.go for why that assumption didn't hold up.
