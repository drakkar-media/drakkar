...
func (c *Client) Libraries(ctx context.Context) ([]Library, error) {
	...
	for _, d := range resp.MediaContainer.Directory {
		out = append(out, Library{Key: d.Key, Title: d.Title, Type: d.Type, Agent: d.Agent})
	}
	return out, nil
}
...
func (c *Client) put(ctx context.Context, path string) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, c.serverURL+path, nil)
	if err != nil {
		return err
	}
	req.Header.Set("X-Plex-Token", c.token)
